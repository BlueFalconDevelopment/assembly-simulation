...
; Stage 10.17 — Pause
;
; 17_pause/, and the first of the new functionality planned with the
; user: ESC or P, in a shift, pauses everything (pause.asm). The war,
; the shift clock, the job's clock, the Bikers and the time of day all
; wait: the main loop skips the update while `paused` is set, and the
; render pass doesn't age effects or count down flashes and falls. The
; view is dimmed under a menu:
;
;   RESUME, CONTROLS (every key), OPTIONS (a placeholder until the
;   music), QUIT TO TITLE (keep what the shift earned, lose the
;   package, no death penalty)
;
; W/S choose, E, SPACE or ENTER picks, ESC or P goes back. The font
; gains '<'.
;
; Watch mode never pauses: the same game as 10.16.
;
; Where things are:
...
%include "pause.asm"          ; the pause menu (10.17)
