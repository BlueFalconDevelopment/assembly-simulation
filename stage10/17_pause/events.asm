...
drop_and_book:
    push rbx
    mov ebx, edi
    call med_drop                 ; now and then, weed (10.12)
    imul eax, ebx, Soldier_size
    lea r10, [soldiers]
    add r10, rax
...
update_events:
    ...
    call update_police
    call update_dog
    call update_bikers            ; (10.14; game mode)
    pop r15
    pop r14
    pop r13
    pop r12
...
update_police:
    ...
    je .up_odds
    mov edi, COP_CHANCE_GAME      ; game mode: sooner (10.13)
.up_odds:
    call chance
    test eax, eax
    jz .up_done
    cmp dword [game_mode], 0
    je .up_lane
    call cop_spawn_road           ; game mode: on the road network
    test eax, eax
    jz .up_done
    jmp .up_arrived
.up_lane:
...
.up_drive:
    ...
    jmp .up_moved
.up_road_uturn:
    call cop_uturn                ; (10.13: into the other lane)
    jmp .up_moved
.up_move:
    mov dword [cop_moved], 1
    mov dword [cop_wait], 0
    mov eax, [cop_vel]
    add [cop_rect], eax
    mov eax, [cop_vel + 4]
    add [cop_rect + 4], eax
.up_moved:
    ; game mode: turns, dead ends, off duty (10.13)
    cmp dword [game_mode], 0
    je .up_edges
    call cop_road_step
    test eax, eax
...
.up_touch:
    ...
    call drop_weapon
.up_no_drop:
    ; (10.11) a turf crew gets a replacement all the same; (10.13) so
    ; does everyone in game mode: the war never ends, and with the
    ; police about town (20 arrests every 8 minutes) the gangs would
    ; dwindle over a long session
    cmp dword [game_mode], 0
    jne .up_crew_arrested
    cmp byte [soldier_crew + rbx], 0
    jge .up_crew_arrested
    lea rcx, [respawn_timer]
    mov dword [rcx + rbx*4], 0
    lea rcx, [lives_left]
    mov dword [rcx + rbx*4], 0
    jmp .up_arrested
.up_crew_arrested:                ; (and game mode's)
    lea rcx, [respawn_timer]
    mov dword [rcx + rbx*4], RESPAWN_TICKS
.up_arrested:
    mov edi, ebx
...
bike_box:
    imul eax, [veh_type], VehicleType_size   ; (10.15: whichever you ride, its size;
    lea rcx, [vehicle_types]                 ; VEH_TYPE, written out: vehicles.asm
    add rax, rcx                             ; comes later)
    mov edx, [rax + VehicleType.size]
    mov ecx, edx
    mov eax, edx
    shr eax, 1
    mov edi, [veh_x]
...
update_dog:
    ...
    cmp dword [game_mode], 0
    je .ud_odds
    mov edi, DOG_CHANCE_GAME      ; game mode: sooner (10.13)
.ud_odds:
    call chance
    test eax, eax
    jz .ud_done
    cmp dword [game_mode], 0
    je .ud_edge_walk
    call dog_spawn_road           ; game mode: a sidewalk in town
    test eax, eax
    jz .ud_done
    jmp .ud_leashed
.ud_edge_walk:
...
.ud_check_end:
    ...
    cmp dword [game_mode], 0
    je .ud_edge_end
    call dog_walk_over            ; game mode: the run's ends (10.13)
    test eax, eax
    jnz .ud_over
    jmp .ud_done
.ud_edge_end:
