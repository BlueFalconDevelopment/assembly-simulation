...
bfs_ensure:
    ; a crew's field (10.11) was searched whole: nothing to do
    lea rax, [field_crew]
    cmp rsi, rax
    jb .be_search
    lea rax, [field_crew_end]
    cmp rsi, rax
    jb .be_whole
.be_search:
    push rbx
...
.bf_soldier:
    ...
    jz .bf_soldier_next
    lea rax, [soldiers + PLAYER * Soldier_size]
    cmp rbx, rax
    je .bf_soldier_next           ; you're chased up close only, not across
                                  ; the map by the flow fields (10.05)
    ; nor are the turf crews (10.11): the war doesn't steer toward
    ; them, and 30 sources scattered round the map made every search
    ; spread from each of them (headless wars 55% slower)
    mov rax, rbx
    lea rcx, [soldiers]
    sub rax, rcx
    xor edx, edx
