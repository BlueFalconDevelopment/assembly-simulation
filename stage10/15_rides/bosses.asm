...
.ub_alert_done:
    xor ebx, ebx                  ; gang
.ub_team:
    ; the endless war (10.04): once a gang's Big Homie is dead, he can
    ; come out again -- when the gang falls BOSS_KILL_GAP kills further
    ; behind than it was when he died (boss_base), so not straight away
    cmp dword [game_mode], 0
    je .ub_state
    cmp dword [boss_state + rbx*4], 2
    jne .ub_state
    imul eax, ebx, Soldier_size
    lea r10, [soldiers + BOSS0 * Soldier_size]
    cmp dword [r10 + rax + Soldier.health], 0
    jg .ub_state
    mov dword [boss_state + rbx*4], 0
    mov eax, ebx
    xor eax, 1
    mov eax, [score + rax*4]
    sub eax, [score + rbx*4]
    mov [boss_base + rbx*4], eax
.ub_state:
...
.ub_by_kills:
    ...
    mov eax, [score + rax*4]
    sub eax, [score + rbx*4]
    sub eax, [boss_base + rbx*4]  ; (0 in watch mode)
