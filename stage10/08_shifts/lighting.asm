...
.ls_lamp:
    ...
    dec r12d
    jnz .ls_lamp
    ; the gangs' lobbies are lit, and it spills out of their doors
    ; (10.03: the homes' sites; a closed site stays dark)
    ;   r12d gang   r15d its site   rbx lobby, then door   r14d doors left
    xor r12d, r12d
.ls_home:
    mov r15d, [home + r12*4]
    mov eax, r15d
    ...
    mov ecx, [rax + r15*8]        ; its first door
    mov r14d, [rax + r15*8 + 4]   ; how many
    shl ecx, 4
    lea rbx, [site_doors]
    add rbx, rcx
.ls_door:
    mov edi, [rbx + 8]
    shr edi, 1
    add edi, [rbx]                ; the doorway's centre
    mov esi, [rbx + 12]
    shr esi, 1
    add esi, [rbx + 4]
    lea rdx, [kern_mid]
