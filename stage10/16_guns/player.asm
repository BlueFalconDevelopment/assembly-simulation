...
; the player's RNG, the zoom, and the save. You're not in the war yet:
; the title screen is (shifts.asm; 10.08)
player_start:
    sub rsp, 8
...
.ps_seeded:
    ...
    call save_setup
    call load_save
    call meds_start               ; the dispensaries (10.12)
    call paint_clubhouse          ; the Bikers' (10.14)
...
.psp_take:
    ...
    mov eax, [player_max_hp]      ; (10.10: the shop's gear)
    mov [r10 + Soldier.health], eax
    mov [player_hp_was], eax
    mov dword [r10 + Soldier.cooldown], 0
    mov dword [r10 + Soldier.team], FACTION_PLAYER
    call player_loadout           ; your weapons, the pistol in hand (10.16)
...
update_player:
    ...
    call update_meds              ; weed: age, restock, pick up (10.12)
    call update_nades             ; grenades in the air, blasts (10.16)
    ; ---- dead? ----
...
.up_counting:
    ...
    jg .up_ret
    mov edi, 1                    ; dying ends the shift (10.08)
    call shift_end
    jmp .up_ret
.up_alive:
...
player_fire:
    ...
    mov eax, [player_weapon]      ; (10.16: its row of pgun)
    PGUN_AT eax
    mov r14d, [rax + PGun.range]
    imul r14d, r14d
    cmp dword [player_weapon], PW_BAT
    jne .pf_range
    ; the bat: whoever's nearest you, within its reach
...
.pf_have_target:
    ...
    mov eax, [player_weapon]      ; (10.16: the table; the shotgun's
    PGUN_AT eax                   ; row is its far figures)
    mov r13d, [rax + PGun.dmg]
    mov r14d, [rax + PGun.hit]
    mov r15d, [rax + PGun.cd]
    cmp dword [player_weapon], PW_SHOTGUN
    jne .pf_odds
...
.pf_steady:
    ...
    cmp dword [player_weapon], PW_BAT
    je .pf_struck                 ; a swing hits only him
    mov edi, PLAYER
    mov esi, ebx
...
.pf_rolled:
    mov eax, [player_weapon]
    PGUN_AT eax
    mov edi, [rax + PGun.looks]   ; its tracer, blast or swing
    mov esi, PLAYER
    mov edx, ebx
    mov ecx, r12d
    call spawn_effect
...
pf_spend:
    mov eax, [player_weapon]
    cmp eax, PW_BAT
    je .ps_cool                   ; (10.16: the bat's never spent)
    lea rcx, [player_ammo]
