...
.dh_boss_gang:
    ...
    jmp .dh_middle
.dh_no_boss:
    ; the Bikers ride out: a warning, even in a shift (10.14)
    cmp dword [biker_alert], 0
    jle .dh_no_bikers
    mov r12d, COLOR_CLUB
    lea rsi, [hud_bikers]
    mov edx, hud_bikers_len
    call append_bytes
    jmp .dh_middle
.dh_no_bikers:
    ; in a shift your line stays up: you can see the police and the dog
    ; (10.13: they're out most of the time now)
...
.dh_you:
    ...
    mov edx, hud_hp_len
    call append_bytes
    call append_hp                ; "120/150 (8 HITS)" (10.12)
    call append_weapon            ; "   RIFLE 10" (10.16)
    lea rsi, [hud_kills]
    ...
    cmp dword [riding], 0
    je .dh_off_bike
    lea rsi, [hud_gap]            ; (10.15: whichever you ride)
    mov edx, 3
    call append_bytes
    imul eax, [veh_type], VehicleType_size   ; (VEH_TYPE, written out:
    lea rcx, [vehicle_types]                 ; vehicles.asm comes later)
    add rax, rcx
    mov rsi, [rax + VehicleType.label]
    mov edx, [rax + VehicleType.label_len]
    call append_bytes
    mov byte [rdi], ' '
    inc rdi
    mov esi, [veh_health]
