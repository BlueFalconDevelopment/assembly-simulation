...
; and ENTER to start the shift. Three pages: GEAR, GUNS (10.16), and
; RIDES (10.15) -- the vehicles, bought once and kept; you pick which to
; ride this shift. Each item says which page it's on.
;
;   A / D (or the arrows)  the page before, the page after
...
struc ShopItem
    ...
    .page:  resd 1            ; PAGE_GEAR or PAGE_GUNS (10.16)
endstruc
...
; SHOP_ITEM "name", "what it does", max level, price 1, 2, 3, page
; (the strings go to .rodata; the row stays in the table. The order is
; the save file's: new items go at the end)
%macro SHOP_ITEM 7
    %if %3 > 3
...
section .data
    shop_items:
    SHOP_ITEM "BODY ARMOR",  "TAKE 15% LESS DAMAGE A LEVEL",   3, 150, 300, 500, PAGE_GEAR
    SHOP_ITEM "TOUGHNESS",   "+25 MAX HEALTH A LEVEL",         3, 100, 200, 350, PAGE_GEAR
    SHOP_ITEM "BIG MAGS",    "+30 PISTOL ROUNDS A LIFE",       3,  60, 120, 200, PAGE_GUNS
    SHOP_ITEM "SHOTGUN",     "START EACH LIFE WITH 12 MORE SHELLS", 2, 250, 200, 0, PAGE_GUNS
    SHOP_ITEM "HEAVY FRAME", "+30 HEALTH FOR YOUR RIDE A LEVEL", 2, 120, 240, 0, PAGE_GEAR
    SHOP_ITEM "PISTOL UPGRADE", "HITS HARDER AND FIRES FASTER. LEVEL 3: ONE SHOT", 3, 200, 400, 800, PAGE_GUNS
    SHOP_ITEM "SMG",         "FAST FIRE. 60 ROUNDS A LIFE A LEVEL", 2, 400, 300, 0, PAGE_GUNS
    SHOP_ITEM "RIFLE",       "LONG RANGE. ONE SHOT DROPS THEM. 10 A LEVEL", 2, 600, 400, 0, PAGE_GUNS
    SHOP_ITEM "BAT",         "FOR WHEN THE AMMO RUNS OUT", 1, 100, 0, 0, PAGE_GUNS
    SHOP_ITEM "GRENADES",    "2 A LIFE A LEVEL. MIND THE BLAST", 3, 300, 300, 300, PAGE_GUNS
    ...
    t_shop_guns db "THE SHOP: GUNS"
    t_shop_guns_len equ $ - t_shop_guns
    ...
    shop_page   dd 0                     ; PAGE_GEAR, PAGE_GUNS or PAGE_RIDES
...
SK_DOWN equ 2
SK_BUY  equ 4
SK_PAGE equ 8                 ; the next page
SK_BACK equ 16                ; the page before
MSG_BOUGHT  equ 1
MSG_BROKE   equ 2
MSG_MAXED   equ 3
MSG_RIDING  equ 4
...
apply_gear:
    ...
    mov [pgun + PGun.dmg], ecx    ; (10.16: the pistol's row of the gun table)
    mov ecx, [pistol_cd_by_level + rax*4]
    mov [player_pistol_cd], ecx
    mov [pgun + PGun.cd], ecx
    ; the rest of your weapons (10.16)
    movzx eax, byte [shop_levels + SHOP_SMG]
    imul eax, eax, SMG_PER_LEVEL
    mov [player_smg], eax
    movzx eax, byte [shop_levels + SHOP_RIFLE]
    imul eax, eax, RIFLE_PER_LEVEL
    mov [player_rifle], eax
    movzx eax, byte [shop_levels + SHOP_NADES]
    imul eax, eax, NADES_PER_LEVEL
    mov [player_nades], eax
    movzx eax, byte [shop_levels + SHOP_BAT]
    mov [player_bat], eax
...
shop_open:
    ...
    mov dword [shop_keys_was], SK_UP | SK_DOWN | SK_BUY | SK_PAGE | SK_BACK   ; held keys don't count
    ret
...
.us_up:
    test ebx, SK_PAGE | SK_BACK
    jz .us_rows
    mov eax, [shop_page]
    inc eax                       ; the next page
    test ebx, SK_PAGE
    jnz .us_page
    add eax, SHOP_PAGES - 2       ; the one before
.us_page:
    xor edx, edx
    mov ecx, SHOP_PAGES
    div ecx
    ...
    mov dword [shop_msg_left], 0
.us_rows:
    call page_rows                ; this page's rows
    mov ecx, eax
    test ebx, SK_UP
    jz .us_dn
    dec dword [shop_pick]
...
.us_try:
    test ebx, SK_BUY
    jz .us_done
    cmp dword [shop_page], PAGE_RIDES
    jne .us_gear
    call ride_buy
    jmp .us_said
.us_gear:
...
shop_buy:
    sub rsp, 8
    mov edi, [shop_pick]
    call page_item                ; (10.16: the row, on this page)
    mov edx, eax
    SHOP_ROW
    mov ecx, [rax + ShopItem.max]
...
SHOP_ROWS_MAX equ 7           ; the most rows a page has (GUNS, 10.16): what
                              ; it says and the keys go under that
...
draw_shop:
    ...
    jmp .ds_titled
.ds_guns_title:
    TEXT_LINE t_shop_guns, t_shop_guns_len, SHOP_TOP, COLOR_TITLE
    jmp .ds_titled
.ds_rides_title:
...
.ds_titled:
    ...
    jmp .ds_keys
.ds_gear:
    ; the list: "> BODY ARMOR    1/3    $300" -- this page's items
    ; (ebx the row; r12d, until the end, the item)
    xor ebx, ebx
.ds_row:
    mov edi, ebx
    call page_item
    ...
    SHOP_ROW
    mov r14, rax
    movzx r15d, byte [shop_levels + r12]
    lea rdi, [hud_buf]
    mov byte [rdi], ' '
    cmp ebx, [shop_pick]
    jne .ds_mark
...
.ds_msg:
    ...
    mov edx, t_maxed_len
.ds_say:
    mov ecx, SHOP_LIST + SHOP_ROWS_MAX + 1
    call overlay_line
.ds_keys:
    TEXT_LINE t_shop_keys, t_shop_keys_len, SHOP_LIST + SHOP_ROWS_MAX + 3, COLOR_TITLE
    pop r15
    pop r14
    pop r13
    pop r12
...
.dr_desc:
    ...
    call append_uint
.dr_said:
    mov r12d, SHOP_LIST + SHOP_ROWS_MAX + 1
    mov r13d, COLOR_HUD_TEXT
    call overlay_buf
.dr_done:
    pop r15
...
; int page_rows(void) -> eax: how many rows shop_page has (the rides:
; VEH_TYPES; else its items). A leaf
page_rows:
    mov eax, VEH_TYPES
    cmp dword [shop_page], PAGE_RIDES
    je .pr_done
...
.pr_item:
    cmp edx, SHOP_COUNT
    jae .pr_done
    imul ecx, edx, ShopItem_size
    lea r8, [shop_items]
    mov ecx, [r8 + rcx + ShopItem.page]
    cmp ecx, [shop_page]
    jne .pr_next
    inc eax
.pr_next:
...
; int page_item(row: edi) -> eax: which of shop_items is that row of
; shop_page. A leaf
page_item:
    xor eax, eax
.pi_item:
...
.pi_next:
    ...
    jmp .pi_item
.pi_none:
    xor eax, eax                  ; (can't happen: rows come from page_rows)
.pi_done:
    ret
