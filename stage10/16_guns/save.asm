...
;   24 best shift ($)   28.. the shop's levels, a byte an item (10.10)
;   56 the rides you own, a bit each   57 the one you ride (10.15)
;   58..59 reserved
...
SAVE_GEAR   equ 28            ; the shop's levels (10.10; old saves: zeros)
SAVE_RIDES  equ 56            ; the rides you own (a bit each), and the one
                              ; you ride (10.15; old saves: zeros)
...
load_save:
    ...
    mov eax, [save_buf + 24]
    mov [best_shift], eax
    lea rsi, [save_buf + SAVE_GEAR]
    lea rdi, [shop_levels]
    mov ecx, SHOP_COUNT
    cld
    rep movsb
    call shop_clamp
    ; the rides: the bicycle's always yours; riding one you don't own
    ; (a damaged-but-checksummed save can't, but still) means the bicycle
    movzx eax, byte [save_buf + SAVE_RIDES]
    and eax, (1 << VEH_TYPES) - 1
    or eax, 1
    mov [ride_owned], al
    movzx ecx, byte [save_buf + SAVE_RIDES + 1]
    cmp ecx, VEH_TYPES
    jae .ls_bicycle
    bt eax, ecx
    jc .ls_ride
...
write_save:
    ...
    mov [save_buf + 24], eax
    lea rsi, [shop_levels]
    lea rdi, [save_buf + SAVE_GEAR]
    mov ecx, SHOP_COUNT
    rep movsb
    mov al, [ride_owned]
    mov [save_buf + SAVE_RIDES], al
    mov al, [ride_pick]
    mov [save_buf + SAVE_RIDES + 1], al
    SAVE_SUM
    mov [save_buf + 60], eax
    mov eax, SYS_OPEN
    lea rdi, [save_tmp]
