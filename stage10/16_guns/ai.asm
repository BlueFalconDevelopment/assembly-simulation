...
; (10.10: in game mode, the game's name instead)
build_title:
    lea rdi, [title_buf]
    cmp dword [game_mode], 0
    je .bt_stage
    lea rsi, [game_name]
    mov edx, game_name_len
    call append_bytes
    mov byte [rdi], 0
    ret
.bt_stage:
...
.scan_loop:
    ...
    ; the player is only worth chasing up close: beyond PLAYER_AGGRO the
    ; gangs let you go and get back to each other (10.05)
    cmp dword [rbp + FNE_J], PLAYER
    jne .fne_not_player
    cmp ecx, PLAYER_AGGRO * PLAYER_AGGRO
    jg .scan_next
.fne_not_player:
...
.have_actual:
    ...
    cmp ecx, PLAYER
    jae .update_next               ; you move yourself (update_player, 10.05);
                                   ; the Bikers ride (update_bikers, 10.14)
    mov eax, ecx
    imul eax, Soldier_size
    lea r10, [soldiers]
    add r10, rax
...
.cooldown_done:
    ...
    mov dword [us_flee], 0
    mov qword [us_field], 0        ; (10.11: a crew's own field)
...
.no_fear:
    ...
    ; ---- a turf crew (10.11, game mode): its post, not the war ----
    mov eax, [rbp + US_ACTUAL]
    cmp byte [soldier_crew + rax], 0
    jl .not_crew
    mov edi, eax
    call crew_think
    cmp eax, -1
    jl .update_next                ; at its spot
    je .crew_walk
...
.crew_walk:
    ...
    mov eax, [crew_gy]
    mov [rbp + US_GOAL_Y], eax
    mov rax, [crew_field]          ; round walls: its crew's field
    mov [us_field], rax
    jmp .do_move
.not_crew:
...
.ff_tallied:
    ...
    mov edi, [rbp + US_TARGET]
    mov esi, r12d
    call armor_damage             ; (10.10: your body armor)
    sub dword [r11 + Soldier.health], eax
...
.no_respawn:
    ; now and then, a bottle of weed where he fell (10.12; a shift only,
    ; on your RNG)
    mov edi, [rbp + US_TARGET]
    call med_drop
    mov eax, [rbp + US_TARGET]
    ...
    cmp eax, WEAPON_KNIFE
    je .update_next
    cmp dword [rbp + US_TARGET], PLAYER
    je .update_next                ; your pistol goes with you (10.05)
...
.follow_field:
    ...
    mov rsi, [us_field]            ; a crew walking back to its post
    test rsi, rsi
    jnz .have_field
    lea rsi, [field_pk]
    cmp dword [rbp + US_IS_PICKUP], 0
