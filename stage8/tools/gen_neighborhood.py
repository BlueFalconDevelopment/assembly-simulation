...
def dressing():
    ...
    for k, (rect, t, doors) in complexes.items():
        ...
        for dr in door_rects(rect, t, doors):
            r.append(dr + (C['mat'],))
    # props
    # cars: the pixel art from gen_sprites.py (CAR_ART, facing east;
    # turned to face south for the upright ones), in each car's colour,
    # as runs of same-coloured pixels
        ...
        for yy, row in enumerate(g):
            ...
            while xx < len(row):
                ...
                while run < len(row) and row[run] == v:
                    run += 1
                if v:
                    r.append((x + xx, y + yy, run - xx, 1, pal[v]))
                xx = run
