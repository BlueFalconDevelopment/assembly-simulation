...
struc Soldier
    ...
    .target:   resd 1
    .cooldown: resd 1
    .avoid_dir: resd 1   ; 0 = prefer negative (up/left) side-step
                            ; direction first when blocked, 1 = prefer
                            ; positive (down/right) -- sticky per-soldier,
...
.try_horizontal:
    ; same sticky-preference idea, left (negative) vs right (positive)
    mov edi, [rbp + US_SELF_X]
    sub edi, MOVE_SPEED
    mov esi, [rbp + US_SELF_Y]
    call is_box_blocked
    mov [rbp + US_NEG_BLOCKED], eax          ; "left" blocked?

    mov edi, [rbp + US_SELF_X]
    add edi, MOVE_SPEED
    mov esi, [rbp + US_SELF_Y]
    call is_box_blocked
    mov [rbp + US_POS_BLOCKED], eax          ; "right" blocked?
