...
; 13 — The neighborhood
;
; 12_respawn.asm, moved off the test arenas and into a city
; neighborhood: two rival gangs, the Crips (blue, team 0) and the
; Bloods (red, team 1), each coming out of its own apartment complex.
;
;   - 1280 x 720 (plus the scoreboard strip). Streets, blocks of
;     buildings, a parking lot, alleys and row houses. The layout is
;     data, generated and checked by tools/gen_neighborhood.py and
;     pasted between the MAP DATA markers below
;   - not mirrored. Instead, each game randomly decides which complex
;     is whose (choose_sides), so any home-turf advantage averages out
;     over many games, and the win line says where the Crips lived so
;     it can be measured
;   - soldiers spawn, and respawn, inside their complex's lobby and walk
;     out through its doors
;   - buildings and complex walls block walking AND bullets. Cars,
;     dumpsters and fences are low cover: they block walking but you
;     can shoot over them. Both are precomputed once into `blockmap`,
;     one byte per possible soldier corner position, which also turns
;     every collision test into a single lookup
;   - the look (asphalt, lane markings, sidewalks, rooftops, cars...)
;     is a list of coloured rectangles, drawn once into bg_buffer and
;     copied to the screen each frame. Drawing only
;   - default rules: 3 lives per soldier, last gang standing
;     (LIVES=3, SCORE_LIMIT=0). LIVES, RESPAWNS and SCORE_LIMIT still
;     override them
; ============================================================
default rel
global main
...
MAX_PICKUPS   equ map_pickups_count
PICKUP_SIZE   equ 10
...
;; ---- MAP DATA (generated by tools/gen_neighborhood.py; don't edit by hand) ----
    ...
    bg_rects:
        ...
        dd 305, 90, 20, 40, 0xFF2828AA
        dd 308, 98, 14, 6, 0xFF503C32
        dd 308, 118, 14, 5, 0xFF503C32
        dd 325, 470, 20, 40, 0xFF964628
        dd 328, 478, 14, 6, 0xFF503C32
        dd 328, 498, 14, 5, 0xFF503C32
        dd 905, 250, 20, 40, 0xFF46C8DC
        dd 908, 258, 14, 6, 0xFF503C32
        dd 908, 278, 14, 5, 0xFF503C32
        dd 925, 640, 20, 40, 0xFF8C3278
        dd 928, 648, 14, 6, 0xFF503C32
        dd 928, 668, 14, 5, 0xFF503C32
        dd 120, 337, 40, 20, 0xFFE6E6E6
        dd 128, 340, 6, 14, 0xFF503C32
        dd 148, 340, 5, 14, 0xFF503C32
        dd 1150, 363, 40, 20, 0xFF5A8C5A
        dd 1158, 366, 6, 14, 0xFF503C32
        dd 1178, 366, 5, 14, 0xFF503C32
        dd 640, 607, 40, 20, 0xFF285A96
...
; Build and run:
;   make
;   ./build/13_neighborhood                   # 3 lives, last gang standing
;   LIVES=0 SCORE_LIMIT=200 ./build/13_neighborhood   # (LIVES=0: no
;                                             # limit) respawns, first to 200
;   HEADLESS=1 SEED=0x1234 ./build/13_neighborhood
;   STAGGER=0 ./batch.sh 48                   # headless, 4 at a time
; To change the map: edit tools/gen_neighborhood.py, then
;   python3 tools/gen_neighborhood.py --write 13_neighborhood.asm
;
; Questions to answer by experimenting:
;   - The win line ends "; crips home west" or "east". Batch a few
;     hundred games and count wins by home, not by gang. Is one complex
;     better? Why might it be (doors, distance to the pickups)?
;   - Make cars block bullets too (move them from map_props to
;     map_walls in the generator). Do games get longer?
;   - bg_buffer costs a 3.7 MB copy every frame. What would drawing
;     only the parts soldiers moved over save, and what would it cost?
; ------------------------------------------------------------
