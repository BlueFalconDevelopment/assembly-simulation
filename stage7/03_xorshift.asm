; ============================================================
; 03 — A hand-rolled xorshift RNG
;
; 02_random_spawn.asm with libc's rand()/srand()/time() replaced by
; about 40 lines of our own:
;
;   rng_seed  -- read the CPU's timestamp counter (rdtsc) and run it
;                through a splitmix64 mixing step into rng_state
;   rng_next  -- one xorshift64 step (shifts 13, 7, 17), returns the
;                TOP 32 bits of the new state
;
; Every call site just changes `call rand` to `call rng_next`. The
; callers only ever did `and eax, 1` or an unsigned `div`, so a full
; 32-bit unsigned result (instead of rand()'s 0..RAND_MAX) is fine.
;
; Side effect worth having: rdtsc changes billions of times a second,
; so two games launched in the same second no longer get the same seed.
; stage6c's batch harness had to stagger launches a second apart
; purely because of srand(time(NULL)).
; ============================================================
default rel
global main
...
section .data
    title db "Stage 7.03 - xorshift RNG", 0
