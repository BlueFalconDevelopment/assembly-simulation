; ============================================================
; 01 — Soldier-vs-soldier collision
;
; stage6c/01_scale_up.asm, plus: no two living soldiers can overlap.
; At 50v50 the crowds at the wall's gap used to stack a dozen soldiers
; onto the same few pixels.
;
; One new function, is_spot_blocked(self, x, y): walls/screen edge (via
; is_box_blocked) OR any other living soldier's box. Every candidate
; position in update_soldiers' .do_move now goes through it. line_blocked
; (wall routing AND line of sight) stays walls-only on purpose.
;
; A step blocked by a soldier tries the main axis alone, then falls
; back to the existing sticky side-step -- never a minor-axis slide,
; which would undo the side-step on the next tick (see .do_move).
; ============================================================
default rel
global main
...
section .data
    title db "Stage 7.01 - soldier collision", 0
...
;   ./batch.sh 48
;
; Watch the gap: soldiers now queue up and flow around each other
; instead of merging into single squares.
;
; Questions to answer by experimenting:
;   - In .do_move, after the main-axis try fails, try the MINOR axis
;     alone before jumping to .side_step. Rebuild and watch a crowd at
;     the gap closely (or trace one blocked soldier's y every tick).
;     What do you see, and why doesn't the sticky avoid_dir save it?
;   - is_spot_blocked skips dead soldiers. What would change if bodies
;     blocked movement instead? Would the gap ever fill up completely?
;   - Knife range is 20 and soldiers are 16px. Work out the closest two
;     soldiers can get on a pure diagonal. Is it inside knife range?
;     What in .do_move keeps a diagonal knife approach from stalling?
; ------------------------------------------------------------
