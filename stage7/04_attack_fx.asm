; ============================================================
; 04 — Attack animations
;
; 03_xorshift.asm plus something to look at when a soldier attacks.
; Before this, a hit only showed up as the target vanishing when it
; died.
;
;   knife   -- a short blade thrusts from the attacker to the target
;              and pulls back (KNIFE_LIFE frames)
;   pistol  -- a yellow tracer flies from shooter to target
;              (BULLET_TRAVEL frames)
;   shotgun -- three orange pellet tracers in a fan
;   on a hit -- an impact spark at the end of each tracer, and the
;              target flashes white when the blade or bullet arrives
;   on a miss -- tracers veer off to one side and overshoot
;
; All of it is drawing only. spawn_effect records what happened. It
; never calls rng_next and never touches soldier state, so for a given
; seed the fight plays out exactly as in 03. The effects just replay
; each hit or miss on screen a few frames later.
;
; Damage still lands on the tick of the attack, so a soldier killed by
; a bullet is dead before the tracer gets there. To stop bodies
; disappearing before the bullet arrives, dead soldiers stay on screen
; for a few frames (death_linger). That list is only read by the
; drawing code; is_spot_blocked, find_nearest_enemy and check_win still
; check health, so a lingering body blocks nothing and can't be
; targeted.
;
; Also new: draw_line (Stage 3's Bresenham, copied unchanged), and
; fill_rect now clips at the left and top edges too. Before, a
; rectangle with a negative x or y wrote outside the back buffer.
; Nothing drew one until impact sparks, which can land just off the
; left or top edge.
; ============================================================
default rel
global main
...
section .data
    title db "Stage 7.04 - attack animations", 0
