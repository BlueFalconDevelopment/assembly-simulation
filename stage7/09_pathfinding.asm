...
; 09 — Pathfinding
;
; 08_headless.asm, but a soldier whose straight line to its goal is
; blocked by a wall no longer just side-steps and hopes. It follows a
; flow field around the walls.
;
; The field is a grid over soldier corner positions, CELL (9) pixels
; square. build_walkable marks, once per game, each cell where a
; soldier could stand ANYWHERE inside it without touching a wall.
; Then every tick, build_fields runs three breadth-first searches over
; the walkable cells:
;   field_to0  distance to the nearest living team 0 soldier
;   field_to1  distance to the nearest living team 1 soldier
;   field_pk   distance to the nearest weapon pickup
; When line_blocked says the goal can't be walked to directly, the
; soldier looks at the 8 cells around its own and heads for the
; centre of the one with the smallest distance (flow_waypoint). The
; old sticky side-step is now only a fallback, for when no neighbour
; is closer.
;
; Mirror fairness, the thing every earlier stage tripped over:
;   - CELL is 9, not 8. Corners run x = 0..784, and cell k covers
;     [9k-8, 9k]. That maps exactly onto cell 88-k under the mirror,
;     because 784 + 8 is a multiple of 9. With an even cell width, no
;     cell can be centred on the mirror line, and none of this works
;   - BFS distances don't depend on the order cells are visited in,
;     only on the grid, and the grid is symmetric
;   - ties between equally close neighbours go "forward" (toward the
;     enemy's side) first, like the side-step since stage6c
;
; Also a sixth arena: Zigzag, the maze-like layout the greedy side-step
; couldn't handle in 07, back as it was first designed.
;
; Two more changes, both found chasing the stalemates that remained:
;   - SEED=n replays a game exactly, and a stalemate line now ends
;     with "; seed 0x..." so any stuck game from a batch can be
;     replayed and traced
;   - PICKUP_RADIUS 15 -> 24 (SOLDIER_SIZE + 8). A gun holder standing
;     on a pickup, boxed in by knife-wielding teammates who all wanted
;     it, was a permanent deadlock: the grab radius was smaller than a
;     body, so nobody else could reach the spot
; ============================================================
default rel
global main
...
; Of the (up to) 8 cells around the soldier's own, the walkable one
; with the smallest distance, if it's smaller than the soldier's own
; cell's. Returns its centre in flow_wx/flow_wy. A diagonal only
; counts if both cells it cuts past are walkable too, so the step
; can't clip a wall's corner. Ties go to the first in flow_dirs, with
; dx flipped for team 1: "forward" first, for both teams.
;   r12d cx   r13d cy   r14d forward sign   r15d best   ebx best cell
;   rdi dir index
flow_waypoint:
    push rbx
    push r12
...
.fw_straight:
    ...
    cmp ecx, r15d
    jae .fw_next                  ; strictly closer only: ties keep the first
    mov r15d, ecx
    mov ebx, eax
.fw_next:
    inc edi
...
; Build and run:
;   make
;   ./build/09_pathfinding                    # windowed, random arena
;   ARENA=5 ./build/09_pathfinding            # Zigzag
;   HEADLESS=1 SEED=0x1234 ./build/09_pathfinding
;   STAGGER=0 ./batch.sh 48                   # headless, 4 at a time
;
; Questions to answer by experimenting:
;   - Change CELL to 8. Which assembler check fires, and why can't an
;     even cell width be mirror-symmetric here?
;   - Make flow_waypoint try left before right for both teams (drop the
;     forward sign). Batch Zigzag. Does a bias appear?
;   - The flow field ignores other soldiers. Replay the Zigzag
;     stalemate from 09's README with SEED and watch it windowed. What
;     would a fix need to know that the field doesn't?
; ------------------------------------------------------------
