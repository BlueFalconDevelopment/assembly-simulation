...
; 08 — Headless mode
;
; 07_arenas.asm, plus a way to run a game with no window at all:
;
;   HEADLESS=1 ./build/08_headless
;
; In headless mode main never calls SDL. It runs update_soldiers and
; check_win back to back as fast as the CPU allows -- no drawing, no
; 60 fps frame cap -- prints the win line and exits. Rendering was
; always a pure function of the game state (04 proved the effects
; don't change the fight), so the game itself is the same one: with
; the same seed, a headless run and a windowed run end in
; byte-identical soldiers, pickups and rng_state.
;
; The win line now ends with the game's length in ticks (one tick =
; one update = one frame at 60 fps in a window):
;   Team 0 (blue) wins on Pillars! (friendly fire: 0 hits, 0 kills; held fire 1954 times; 2210 ticks)
;
; A headless game that runs to MAX_TICKS stops and reports a
; stalemate instead, so batch.sh no longer has to guess from wall-clock
; time whether a game is stuck:
;   Stalemate on Trenches! (friendly fire: ...; 30000 ticks)
;
; The windowed game is unchanged (no tick limit: you can watch a
; stalemate for as long as you like).
; ============================================================
default rel
global main
...
; Build and run:
;   make
;   ./build/08_headless                       # windowed, random arena
;   HEADLESS=1 ./build/08_headless            # no window, runs flat out
;   HEADLESS=1 ARENA=2 ./build/08_headless
; batch.sh sets HEADLESS=1 itself:
;   STAGGER=0 ./batch.sh 48
;
; Questions to answer by experimenting:
;   - Time one headless game with `time`. How many ticks per second is
;     that, and how many 60 fps frames' worth of game is it?
;   - Profile a headless game with `perf record` / `perf report`.
;     Which function is hottest? (The plan's guess: first_in_line.)
;   - Raise NUM_PER_TEAM until a headless game takes a whole second.
;     Which loop in update_soldiers grows fastest with soldier count?
; ------------------------------------------------------------
