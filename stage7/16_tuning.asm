...
; 16 — Tuning
;
; 15_bighomie.asm, tuned after watching it:
;   - FEAR_RADIUS 220 -> 280, and soldiers now run OFF the road,
;     across the car's direction of travel, instead of straight away
;     from it (which meant running down the lane in front of it). The
;     bigger radius alone made no difference to arrests
;   - the Big Homie comes out earlier: BOSS_AT defaults to 60, not 40.
;     At 40 he usually appeared ~80% of the way through a game, in the
;     last seconds; at 60 it's ~71% (batches in the README)
; and the win line now says when the first Big Homie came out
; ("; big homie at 2100", in ticks), which is how "last minute" was
; measured.
; ============================================================
default rel
global main
...
; Build and run:
;   make
;   ./build/16_tuning                         # Big Homie at 60%
;   BOSS_AT=40 ./build/16_tuning              # later: 15's default
;   BOSS_AT=0 ./build/16_tuning               # no Big Homies
;   HEADLESS=1 SEED=0x1234 ./build/16_tuning
;   STAGGER=0 ./batch.sh 48                   # headless, 4 at a time
;
; Questions to answer by experimenting:
;   - Set FLEE_DIST to 0 for a car going left/right (so they only flee
;     along the road). How many more arrests?
;   - "big homie at" over "ticks" says how late he came. Plot it
;     against BOSS_AT for 30..90. Where does it stop mattering?
;   - Fleeing soldiers side-step walls. Would fleeing along the flow
;     field toward their own complex be better or worse?
; ------------------------------------------------------------
