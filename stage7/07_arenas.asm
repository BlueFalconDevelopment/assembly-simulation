...
; 07 — Arenas
;
; 06_hold_fire.asm, but the wall layout is no longer hard-coded.
; There are NUM_ARENAS layouts in arena_table, and each game picks
; one at random (or ARENA=n picks one, for testing and batching):
;
;   0 Divide      the original: one centre wall with a gap
;   1 Pillars     a staggered grid of small square pillars
;   2 Crossroads  four big blocks, a + of corridors between them
;   3 Trenches    short staggered wall segments
;   4 Outposts    short walls and a centre bunker
;
; Each arena lists only its LEFT-half walls. spawn_obstacles adds the
; mirror image of each one (x' = SCREEN_W - x - w), unless the wall is
; its own mirror (it straddles the centre line). So every arena is
; left-right symmetric by construction, the same way spawns and
; pickups are.
;
; Walls now go where the pickup table spots are, so a pickup that
; lands in a wall slides toward its own team's spawn side (-x on the
; left, mirrored on the right) until a soldier could stand on it.
; Walls stay out of the spawn strips (x < 200 and its mirror), so the
; slide always ends before it gets there.
;
; The win line names the arena:
;   Team 0 (blue) wins on Pillars! (friendly fire: 0 hits, 0 kills; held fire 812 times)
; ============================================================
default rel
global main
...
section .data
    title_prefix db "Stage 7.07 - arenas: "
...
; title_buf = "Stage 7.07 - arenas: <name>", 0-terminated for SDL.
build_title:
    lea rdi, [title_buf]
...
; Build and run:
;   make
;   ./build/07_arenas             # a random arena
;   ARENA=2 ./build/07_arenas     # Crossroads
; Or, to count wins over many games on one arena without watching:
;   ARENA=2 STAGGER=0 ./batch.sh 48
;
; Questions to answer by experimenting:
;   - Add an arena with a wall that crosses the centre line but isn't
;     its own mirror (say x=380, w=60). What does the assembler say,
;     and what would the game have looked like without that check?
;   - Build a U-shaped pocket (three walls) open toward the enemy.
;     Do soldiers that walk into it get out? Why not? (Hint: the
;     side-step only ever moves perpendicular to the goal.)
;   - Change the pickup slide to go +x instead of -x. Which arenas
;     now put weapons closer to the enemy than to you?
; ------------------------------------------------------------
