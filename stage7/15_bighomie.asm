...
; 15 — The Big Homie
;
; 14_events.asm plus a miniboss. Once per gang per game, when that gang
; is clearly losing, its Big Homie walks out of its apartment complex:
;
;   - he comes out when the gang's remaining strength (soldiers alive,
;     soldiers waiting to respawn, and every life still left) drops
;     below BOSS_AT percent of the other gang's -- 40 by default, or
;     BOSS_AT=n in the environment (0 = no Big Homies). With unlimited
;     lives there's nothing to count, so it's BOSS_KILL_GAP kills
;     behind instead. Not while the other gang's Big Homie is still
;     alive: the leader can't answer one with its own
;   - BOSS_HEALTH health (a soldier has 100), already armed with a
;     pistol, double damage and BOSS_HIT_BONUS more chance to hit.
;     Not scared of the police (who can still arrest him). One life
;   - a gold border and a health bar; the scoreboard announces him
;
; Each gang's Big Homie has his own soldier slot after the 50-a-side
; squads (TOTAL_SOLDIERS is now SQUAD + 2), so nothing about the squads
; changes. Two spare pickup slots keep 6b's "every weapon is either
; held or lying in exactly one slot" true when he drops his gun.
; ============================================================
default rel
global main
...
BOSS_TRIGGER_PCT equ 40      ; default for BOSS_AT (40 gave the most
                             ; comebacks of 40/60/80: see the README)
...
.draw_done:
    ...
    call draw_bosses               ; gold border and health bar
...
; void draw_bosses(void) -- a gold border and a health bar on each Big
; Homie who's out (drawing only)
draw_bosses:
    push rbx
    push r12
    push r13
...
.de_walker:
    ...
    je .de_done
    ; the walker: a jacket and a head
    lea rdi, [back_fb]
    mov esi, [walker_x]
    mov edx, [walker_y]
    mov ecx, WALKER_SIZE
    mov r8d, WALKER_SIZE
    mov r9d, COLOR_WALKER
    call fill_rect
    lea rdi, [back_fb]
    mov esi, [walker_x]
    add esi, 2
...
; Build and run:
;   make
;   ./build/15_bighomie                       # Big Homie at 40%
;   BOSS_AT=60 ./build/15_bighomie            # earlier: at 60%
;   BOSS_AT=0 ./build/15_bighomie             # no Big Homies
;   HEADLESS=1 SEED=0x1234 ./build/15_bighomie
;   STAGGER=0 ./batch.sh 48                   # headless, 4 at a time
;
; Questions to answer by experimenting:
;   - Batch BOSS_AT=40, 60 and 80. How often does the gang that got its
;     Big Homie first come back and win? Which setting makes the
;     closest games?
;   - Give him a shotgun instead of a pistol. Better or worse?
;   - The win line says "; big homies 1-0". What does it mean when both
;     gangs got one?
; ------------------------------------------------------------
