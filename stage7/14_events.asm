...
;
; 13_neighborhood.asm plus two things the gangs don't control:
;
; THE POLICE. Every so often (COP_CHANCE, never more than one at a
; time) a police car drives the length of the avenue or one of the
; cross streets, in the proper lane, and leaves at the far edge.
;   - the officers shoot, every COP_FIRE_TICKS, at the nearest soldier
;     within COP_RANGE they can see (walls block them; cars don't)
;   - any soldier within FEAR_RADIUS of the car drops everything and
;     runs straight away from it
;   - a soldier the car touches is ARRESTED: gone for good, no
;     respawn, whatever lives it had left
;
; THE PITBULL. Someone walks a dog along one of the avenue's
; sidewalks. At a random moment it slips its leash, chases the
; nearest soldier of either gang and bites (DOG_BITE_*), until animal
; control collects it DOG_RAGE_TICKS later.
;
; Police and dog kills score for nobody, and those soldiers respawn
; as usual. The scoreboard shows POLICE! or DOG LOOSE! while it's
; happening; the win line adds arrests, police kills and dog kills.
; ============================================================
default rel
global main
...
; Build and run:
;   make
;   ./build/14_events                         # 3 lives, last gang standing
;   HEADLESS=1 SEED=0x1234 ./build/14_events
;   STAGGER=0 ./batch.sh 48                   # headless, 4 at a time
;
; Questions to answer by experimenting:
;   - Set COP_CHANCE to 60 (a car almost always on the way). How many
;     arrests does a game end with? Does it still end?
;   - Soldiers run straight away from the car and side-step walls. Why
;     can't they use the flow field to flee? What field would they need?
;   - Let the gangs shoot the dog. Where in update_soldiers would it go,
;     and what's the smallest change that makes it a target?
; ------------------------------------------------------------
