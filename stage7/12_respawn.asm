...
; 12 — Respawns
;
; 11_scoreboard.asm, but dead soldiers come back, and the game is won
; on points: the first team to SCORE_LIMIT kills (default 200).
;
; A soldier who dies is back RESPAWN_TICKS (2 s) later, in its own
; team's spawn strip: the same wall-free, mirrored area it started in.
; "Somewhat safe" twice over:
;   - respawn_soldier tries RESPAWN_TRIES random spots in the strip
;     and takes the one farthest from any living enemy
;   - then PROTECT_TICKS (1.5 s) of spawn protection: hits land but do
;     no damage. The soldier blinks while it lasts
;
; Rules, from the environment:
;   RESPAWNS=n      each team gets n respawns in total (unset:
;                   unlimited). A team with nobody alive and no
;                   respawns coming loses, as before
;   LIVES=n         each soldier gets n lives, n-1 respawns (unset
;                   or 0: unlimited). Works with RESPAWNS: a respawn
;                   needs both
;   SCORE_LIMIT=n   first to n kills wins (default 200; 0 = off)
; RESPAWNS=0 is the old game exactly: with the same seed it ends
; byte-identical to 11 (the respawn code never draws a random number
; unless someone actually respawns).
;
; A kill scores for the killer's team (friendly kills score nothing).
; The limit is checked at the moment of the kill, not at the end of
; the tick, so whoever gets there first wins. update_soldiers already
; processes soldiers in a random direction each tick, so "first" is
; fair. The scoreboard shows kills: BLUE 120/200. The win line adds
; the score.
; ============================================================
default rel
global main
...
read_rules:
    sub rsp, 8                    ; align the stack for the libc calls
    ; LIVES=n: every soldier gets n-1 respawns. Unset: unlimited.
...
; RESPAWN_TRIES random spots in the team's spawn strip (mirrored for
; team 1, exactly like spawn_soldiers). Skips spots another soldier
...
.rs_try:
    mov edi, SPAWN_MAX_X - SPAWN_MIN_X + 1
    call rand_range
    lea r13d, [eax + SPAWN_MIN_X]
    mov edi, SPAWN_MAX_Y - SPAWN_MIN_Y + 1
    call rand_range
    lea r14d, [eax + SPAWN_MIN_Y]
    ...
    mov eax, SCREEN_W - SOLDIER_SIZE
    sub eax, r13d
    mov r13d, eax                 ; team 1: the mirrored strip
.rs_side_ok:
    mov edi, ebx
    mov esi, r13d
    mov edx, r14d
...
; Build and run:
;   make
;   ./build/12_respawn                        # respawns, first to 200
;   RESPAWNS=10 ./build/12_respawn            # 10 respawns per team
;   RESPAWNS=0 ./build/12_respawn             # the old game (= 11)
;   LIVES=3 SCORE_LIMIT=0 ./build/12_respawn  # 3 lives each, last team standing
;   SCORE_LIMIT=100 ./build/12_respawn
;   STAGGER=0 ./batch.sh 48                   # headless, 4 at a time
; With respawns unlimited AND SCORE_LIMIT=0 nothing can end the game;
; headless, it stops as a stalemate at MAX_TICKS.
;
; Questions to answer by experimenting:
;   - Set RESPAWN_TRIES to 1 (a random spot, no safety check). Do
;     scores get less close? Watch a Zigzag game: where do soldiers die?
;   - Remove the spawn protection (PROTECT_TICKS 0). How often is a
;     soldier killed within a second of respawning? (Count it: you
;     have protect_timer and the kill code in the same file.)
;   - Zigzag games end far less close (median losing score ~127) than
;     the others (~190). Why would a maze make a lead snowball?
; ------------------------------------------------------------
