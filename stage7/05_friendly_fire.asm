...
; 05 — Friendly fire
;
; 04_attack_fx.asm, except a pistol or shotgun shot now hits whoever
; is actually in the way, not just the soldier it was aimed at.
;
; first_in_line walks the line of fire from the shooter's centre to
; the target's centre, the same Bresenham walk as draw_line and
; line_blocked. At each point it checks whether the point is inside a
; living soldier's box, skipping the shooter's own. The first box it
; enters takes the shot, on either team. If nobody is in the way, that
; is the target, whose box is at the end of the line.
;
; The hit roll, damage and weapon drop are unchanged. They just apply
; to that soldier. There's still exactly one rng_next per attack, so
; the random stream is used the same way as before.
;
; Soldiers don't check for teammates before firing. That's deliberate,
; to see how much it matters. Holding fire would be a separate change.
;
; The knife is unaffected: it is contact range, and the target is the
; only soldier it can reach.
;
; The win line now also reports friendly-fire hits and kills, still as
; a single write(), so batch.sh keeps working:
;   Team 0 (blue) wins! (friendly fire: 14 hits, 3 kills)
; ============================================================
default rel
global main
...
section .data
    title db "Stage 7.05 - friendly fire", 0
...
.have_atk_stats:
    ...
    ; ---- friendly fire: the shot hits whoever is first in the line ----
...
;
; Writes "<msg> (friendly fire: H hits, K kills)\n" in ONE write().
