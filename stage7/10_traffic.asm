...
; 10 — Traffic
;
; 09_pathfinding.asm, but the flow field now steers around other
; soldiers, not just walls.
;
; 09's flow_waypoint returned the single closest neighbouring cell. If
; another soldier stood in the way of that step, the soldier fell back
; to the old sticky side-step. At a chokepoint that could lock up for
; good: two teammates whose best steps crossed each blocked the
; other, and the side-step bounced one of them between two cells
; forever, even though it had an equally good way round.
;
; Now flow_waypoint collects every neighbouring cell that is closer
; to the goal, tries them closest first, and takes the first whose
; next step isn't blocked by another soldier (is_spot_blocked). Only
; if every one is blocked does it return the closest anyway, leaving
; the old fallbacks to deal with it. Ties still go "forward" first,
; so both teams follow the same rule under the mirror.
;
; Everything else is 09: flow fields on a 9px grid, SEED=n replay,
; HEADLESS=1, six arenas.
; ============================================================
default rel
global main
...
; Build and run:
;   make
;   ./build/10_traffic                        # windowed, random arena
;   ARENA=5 ./build/10_traffic                # Zigzag
;   HEADLESS=1 SEED=0x1234 ./build/10_traffic
;   STAGGER=0 ./batch.sh 48                   # headless, 4 at a time
;
; Questions to answer by experimenting:
;   - Let pass 1 also collect neighbours at the SAME distance (sideways
;     moves). Batch Zigzag. Do jams get rarer, or do soldiers start
;     drifting back and forth along equal-distance cells?
;   - Pass 2 calls is_spot_blocked up to 8 times per soldier. Count how
;     often it gets past the first candidate. Is the cost worth it?
;   - Give each soldier a "stuck for N ticks" counter and, past a limit,
;     let it take a sideways or backwards cell. What's the smallest
;     rule that clears a jam without making anyone oscillate?
; ------------------------------------------------------------
