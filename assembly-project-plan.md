...
**Where things stand:** the roadmap (Stages 0–6c) is done, and Stage 7 has fifteen post-roadmap steps. **Latest build: `stage7/15_bighomie.asm`** (~5,600 lines), map generated by `stage7/tools/gen_neighborhood.py`. Start any new change from a copy of it. Working tree clean, `main` in sync with GitHub.
...
1. **Tune the Big Homie further** if wanted: he turns 15% of games round at `BOSS_AT=40`. Levers: `BOSS_HEALTH`, weapon (shotgun?), `BOSS_HIT_BONUS`, `BOSS_AT`.
2. **More random encounters, or tuning these** (the user's preferred direction over strict balance). Everything is a constant at the top of `14_events.asm`: `COP_CHANCE`, `FEAR_RADIUS`, `DOG_*`. Home advantage is now west 54% (13: east 59%); the side swap keeps the gangs even.
3. **Smarter hold-fire repositioning:** step back, or retarget an enemy with a clear line, instead of only side-stepping.
4. **Push the soldier count** using headless mode. `first_in_line` (every box × every point) will be the first hotspot.
5. Weapons scattering a little when dropped on death.
6. **Publish Part 5** (7.09–7.11). The draft is written: `newPOSTS/Learning x86-64 Assembly Part 5 - Pathfinding, Traffic, and a Scoreboard.md` (uncommitted), with the video embed line at the top. Not published yet because the Netlify credits ran out. 7.12–7.13 (respawns, the neighborhood) would make a Part 6.
